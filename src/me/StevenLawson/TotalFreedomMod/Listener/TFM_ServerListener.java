...
public class TFM_ServerListener implements Listener {
    ...
    public void onServerPing(ServerListPingEvent event) {
        ...
        if (TFM_ConfigEntry.ADMIN_ONLY_MODE.getBoolean()) {
            event.setMotd(ChatColor.RED + "BanterFreedom is in admin-only mode.");
            return;
        }

        if (Bukkit.hasWhitelist()) {
            event.setMotd(ChatColor.RED + "BanterFreedom's whitelist is enabled");
            return;
        }

        if (Bukkit.getOnlinePlayers().size() >= Bukkit.getMaxPlayers()) {
            event.setMotd(ChatColor.RED + "BanterFreedom is full.");
            return;
        }
