...
public class Command_bar extends TFM_Command
{
    ...
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        if (args[0].equalsIgnoreCase("clear"))
        {
            ...
            TFM_Util.adminChatMessage(sender, "[Bar Message] The bar was cleared!", false);
        }
        else
        {
            String message = StringUtils.join(ArrayUtils.subarray(args, 0, args.length), " ");
            BarAPI.setMessage(message.replaceAll("&", "§"), 60);
            TFM_Util.adminChatMessage(sender, "[Bar Message] Welcome back to the Bar! (New Message)", false);
        }
        return true;
    }
}
