...
public class Command_fr extends TFM_Command
{
    ...
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        if (args.length == 0)
        {
            ...
            if (allFrozen)
            {
                TFM_Util.adminAction(ChatColor.RED + sender.getName(), "Freezing all players", false);

                setAllFrozen(true);
                playerMsg(ChatColor.RED + "Players are now frozen.");

                for (Player player : Bukkit.getOnlinePlayers())
                {
                    if (!TFM_AdminList.isSuperAdmin(player))
                    {
                        playerMsg(player, ChatColor.YELLOW + "Freedom >" + ChatColor.RED + "You have been frozen due to rulebreakers, you will be unfrozen soon.");
                    }
                }
            }
            else
