...
public class Command_adminmode extends TFM_Command
{
    ...
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        if (args[0].equalsIgnoreCase("off"))
        {
            ...
            TFM_Util.adminAction(sender.getName(), "Opening the server to OPs.", true);
            return true;
        }
        else if (args[0].equalsIgnoreCase("on"))
        {
            TFM_ConfigEntry.ADMIN_ONLY_MODE.setBoolean(true);
            TFM_Util.adminAction(sender.getName(), "Closing the server to OPs.", true);
            for (Player player : server.getOnlinePlayers())
            {
                if (!TFM_AdminList.isSuperAdmin(player))
                {
                    player.kickPlayer(ChatColor.YELLOW + "Freedom >" + ChatColor.RED + "RubyFreedom is currently open to Admins!");
                }
            }
            return true;
        }
